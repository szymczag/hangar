...
verifying the public `0.1.0-rc.8` release.
...
request abuse, while PostgreSQL row-locked budgets enforce concurrent jobs,
active source bytes, and rolling rows independently of cache correctness.
...
For `0.1.0-rc.8`, the trust chain is:

1. signed Git tag `hangar-v0.1.0-rc.8` identifies the source commit;
...
export GIT_TAG=hangar-v0.1.0-rc.8
...
`0.1.0-rc.8` is a prerelease. Only the evaluation profile has completed live
