...
2. confirm the target chart is `0.1.0-rc.34`, its application version is
   `v0.1.0-rc.34`, and its signatures and digests pass the
...
4. deploy every application image as one coordinated Helm revision. Do not mix
   `rc.27` and `rc.34` web or API images because their preview-execution request
...
application component to `rc.34` promptly.
