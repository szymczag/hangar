...
`rc.47` and `rc.51` versions are unsupported because their workshop contracts
differ.
...
### Upgrade from `rc.27` to `rc.51`

`rc.28` was consumed by an incomplete publication and is not an upgrade target.
Releases `rc.31` through `rc.38` are retired. Upgrade directly to `rc.51`.
...
2. confirm the target chart is `0.1.0-rc.51`, its application version is
   `v0.1.0-rc.51`, and its signatures and digests pass the
...
4. deploy every application image as one coordinated Helm revision. Do not mix
   `rc.27` and `rc.51` web or API images because their preview-execution request
...
application component to `rc.51` promptly.
