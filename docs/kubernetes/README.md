...
The current release is `0.1.0-rc.15`. It is qualified for evaluation on
...
The `0.1.0-rc.15` qualification boundary is:
...
| ------------------ | -------------------------------------------- |
| Product version    | `v0.1.0-rc.15`                               |
| Helm chart version | `0.1.0-rc.15`                                |
| Git tag            | `hangar-v0.1.0-rc.15`                        |
| OCI chart          | `ghcr.io/szymczag/charts/hangar:0.1.0-rc.15` |

`rc.1` and `rc.2` were consumed by incomplete publication attempts. Do not use
them. `rc.14` is the immediately previous complete publication, but its Headless
UI 2 dropdown panels can make surrounding task content inert and can become
visually open while internally closed. `rc.13` has the earlier Fragment-backed
combo-box failure, and `rc.12` additionally has hydration and transition
failures. Do not deploy or roll back to those releases. `rc.11` remains the
qualified rollback target. Published versions are immutable and are never
repaired in place.

## Documentation

- [Release `v0.1.0-rc.15` notes](../releases/hangar-v0.1.0-rc.15.md) — review
...
The public `rc.15` chart archive, OCI chart, and digest-pinned Hangar images are
