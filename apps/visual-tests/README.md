...
| An element screenshot of a dialog can include the page behind it | the backdrop is 30% opaque (`oklch(0.1482 0.0034 196.79 / 0.3)`), so the corners outside the panel's border radius are seven parts page to three parts backdrop — whether that matters depends on what sits under those corners, so **measure it** by hiding the content and re-photographing the dialog: build-identity's corners moved, duplicate-project's did not |
