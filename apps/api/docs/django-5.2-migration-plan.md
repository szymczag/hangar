...
The Hangar API was audited end-to-end for Django 5.0/5.1/5.2 breaking changes (12 targeted code scans, a full settings audit, a 129-file migration scan, and per-dependency compatibility research). **The application code is already Django 5.2-clean.** Every breaking-change scan came back unaffected.

The migration is therefore **almost entirely a coordinated third-party dependency bump plus verification** — not a code rewrite. Risk is **low–medium** and concentrated in the dependency layer, not in Hangar's own code.

**No runtime or infrastructure blockers:**

| Prerequisite | Django 5.2 requires | Hangar has                      | Status |
| ------------ | ------------------- | ------------------------------- | ------ |
...
| `manage.py makemigrations --check --dry-run`      | ✅ no changes (after the state-only migration; `0129` in Hangar)               |
| `manage.py migrate` (full history on Postgres 15) | ✅ upstream db `0001`→`0122`; Hangar carries the same no-op state as `0129`    |
...
2. **`plane/db/migrations/0129_alter_draftissue_assignees_alter_issue_assignees_and_more.py`** in Hangar (upstream uses `0122`) — a **state-only** `AlterField` for three `ManyToManyField`s (`issue.assignees`, `draftissue.assignees`, `module.members`) that use `through=`/`through_fields=`. Django 5.1 normalized M2M `through_fields` deconstruction. `sqlmigrate` confirms **all three operations are `(no-op)`** — zero DDL, zero database impact. The Hangar number follows its existing `0122`–`0128` fork migrations and depends on `0128_fileasset_upload_validation_version`. Required only so `makemigrations --check` stays green in CI.
...
- `plane/db/migrations/0129_…` in Hangar (`0122_…` upstream), state-only M2M `AlterField` (Django 5.1 `through_fields` normalization; no DDL).
...
| channels                     | 4.1.0   | **4.3.2**                        | 4.2.1       | 4.3 raises `asgiref>=3.9` (auto-resolved) and makes Daphne an optional extra — Hangar serves ASGI via uvicorn/gunicorn, so no impact.                                                                   |
...
| django-filter                | 24.2    | **25.2**                         | 25.1        | 25.x removed built-in DRF schema gen (Hangar uses drf-spectacular → no impact). 25.2 requires Python ≥3.10 (have 3.12).                                                                                 |
...
| django-redis                 | 5.4.0   | **7.0.0**                        | 6.0.0       | 7.0 renamed zset/hash helper params (not used by Hangar) and drops Django 5.0 (irrelevant). **Verify redis-py floor** (`redis==5.0.4`) satisfies 7.0; if not, pin **6.0.0** instead.                    |
...
| celery                               | 5.4.0      | **5.5.3** (not 5.6.x) | **Override:** 5.6 reverted the SQS→urllib3 change; 5.5.3 is the battle-tested line. Hangar uses Redis broker. Pulls `kombu>=5.5`. (Celery itself isn't Django-pinned — already works on 5.2.)          |
