...
class TestOAuthAvatarSSRF:
    ...
    @patch("plane.authentication.adapter.base.S3Storage")
    @patch(
        "plane.authentication.adapter.base.validate_file_content",
        side_effect=ValueError("invalid image"),
    )
    @patch("plane.authentication.adapter.base.pinned_fetch_following_redirects")
    def test_avatar_rejects_header_only_image_spoof(
        self,
        mock_fetch,
        _validate_content,
        mock_storage,
        ...
        response.iter_content.return_value = [b"MZ\x00\x00"]
        mock_fetch.return_value = (response, None)
        ...
        assert result is None
        mock_storage.return_value.upload_file.assert_not_called()

    @patch("plane.authentication.adapter.base.FileAsset.objects.create")
    @patch("plane.authentication.adapter.base.S3Storage")
    @patch(
        "plane.authentication.adapter.base.validate_file_content",
    ...
    @patch("plane.authentication.adapter.base.pinned_fetch_following_redirects")
    def test_avatar_records_validation_marker(
        self,
        mock_fetch,
        _validate_content,
        mock_storage,
        ...
        assert create_kwargs["storage_metadata"]["ValidationVersion"] == UPLOAD_VALIDATION_VERSION
        assert create_kwargs["upload_validation_version"] == UPLOAD_VALIDATION_VERSION
        assert create_kwargs["storage_metadata"]["DetectedContentType"] == "image/png"

    @patch(
        "plane.authentication.adapter.base.FileAsset.objects.create",
    ...
    @patch("plane.authentication.adapter.base.pinned_fetch_following_redirects")
    def test_avatar_deletes_object_when_database_publish_fails(
        self,
        mock_fetch,
        _validate_content,
        mock_storage,
