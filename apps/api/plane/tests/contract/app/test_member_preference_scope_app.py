...
def project(db, workspace, create_user):
    project = Project.objects.create(name="Pref Project", identifier="PR", workspace=workspace, created_by=create_user)
...
class TestMemberPreferenceScope:
    ...
    def test_member_can_read_own_preferences(self, workspace, project):
        ...
        response = _client(member).get(PREF_URL.format(slug=workspace.slug, project_id=project.id, member_id=member.id))
