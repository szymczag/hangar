...
class TestSubIssuesCrossProjectScope:
    ...
    @pytest.mark.django_db
    def test_write_cross_project_sub_issue_ids_rejected(self, session_client, workspace, project_a, parent_a, orphan_b):
        """Even with an in-project parent, foreign sub_issue_ids are rejected.

        Parent A is valid for the URL project, but ``orphan_b`` lives in project B,
        so strict all-or-nothing validation rejects the request and leaves its parent untouched.
        """
