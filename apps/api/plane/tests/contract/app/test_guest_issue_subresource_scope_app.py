...
def project(db, workspace, create_user):
    """A project with restricted guest visibility and an active owner admin."""
...
class TestGuestIssueSubresourceScope:
    ...
    def test_workspace_admin_with_guest_project_role_reads_foreign_issue_attachments(
        self, guest_client, guest, workspace, project, foreign_issue
    ):
        """Workspace admins retain admin visibility regardless of their project role."""
        WorkspaceMember.objects.filter(workspace=workspace, member=guest).update(role=20)
    ...
    def test_unrestricted_guest_reads_foreign_issue_attachments(self, guest_client, workspace, project, foreign_issue):
