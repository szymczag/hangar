...
def other_member_b(db, workspace, project_b):
    ...
    ProjectMember.objects.create(workspace=workspace, project=project_b, member=user, role=15, is_active=True)
    return user
...
def attacker(db, workspace, project_a):
    ...
    WorkspaceMember.objects.create(workspace=workspace, member=user, role=15, is_active=True)
    ProjectMember.objects.create(workspace=workspace, project=project_a, member=user, role=20, is_active=True)
    return user
...
class TestUndecoratedRouteProjectScope:
    ...
    def test_member_can_put_issue(self, session_client, workspace, project_b, issue_b):
        ...
        # Hangar preserves the authenticated full-update implementation.
