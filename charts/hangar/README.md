...
The latest published chart is `0.1.0-rc.41` (`appVersion: v0.1.0-rc.41`). Its
...
`0.1.0-rc.25`, `0.1.0-rc.28`, or `0.1.0-rc.33`; those versions were consumed by
incomplete release attempts. Releases `rc.31` through `rc.38` are retired after
a repository-history privacy correction and are not supported installation,
upgrade, or rollback targets. `rc.40` is the immediately previous retained
GitHub release.
Earlier `rc.12` through `rc.17` additionally contain frontend migration failures.
Use `0.1.0-rc.41`; there is no security-equivalent rollback target among the
...
Release `0.1.0-rc.41` is available from the
[GitHub Release](https://github.com/szymczag/hangar/releases/tag/hangar-v0.1.0-rc.41)
